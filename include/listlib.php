...
function displayProjectList($startswith, $offset, $range, $projlookup, $trovedetail, $descriptiondetail)
{
    ...
    if (mb_strlen($startswith) < 1) {
        $sql = 'SELECT group_id,group_name,unix_group_name,short_description' . ' FROM ' . $xoopsDB->prefix('xf_groups') . ' WHERE type=1 ' . ' AND is_public=1 ' . " AND status='A' " . ' ORDER BY group_name ASC';
    } else {
        $sql = 'SELECT group_id,group_name,unix_group_name,short_description' . ' FROM ' . $xoopsDB->prefix('xf_groups') . " WHERE (group_name regexp '$startswith'";

        if (mb_strlen($startswith) > 2) {
    ...
    for ($i = 0; $i < $rows; $i++) {
        ...
        $content .= '<table border="0" cellpadding="0" width="100%">' . '<tr valign="top"><td colspan="2">';

        if (!$projlookup) {
            $content .= "<a href='" . XOOPS_URL . '/modules/xfmod/project/?' . $row_grp['unix_group_name'] . "'>";
        } else {
            $content .= "<a href=\"#\" onClick=\"window.opener.document.addprojform.form_proj_name.value='" . htmlspecialchars($row_grp['unix_group_name'], ENT_QUOTES | ENT_HTML5) . "';window.close();\">";
        }

        $content .= '<b>' . htmlspecialchars($row_grp['group_name'], ENT_QUOTES | ENT_HTML5) . '</b></a>';
        ...
        if ($descriptiondetail) {
            ...
            $content .= ' - ' . htmlspecialchars($description, ENT_QUOTES | ENT_HTML5);
        }
...
function displayProjectListHeader($startswith, $contains, $range, $offset, $projlookup, $trovedetail, $descriptiondetail, $firsttimeonpage)
{
    $content = '';
...
function displayProjectListNav($startswith, $offset, $range, $projlookup, $trovedetail, $descriptiondetail)
{
    ...
    if (mb_strlen($startswith) < 1) {
        $sql = 'SELECT count(group_id) AS count FROM ' . $xoopsDB->prefix('xf_groups') . ' WHERE type=1 ' . ' AND is_public=1 ' . " AND status='A' ";
    } else {
        $sql = 'SELECT count(group_id) AS count FROM ' . $xoopsDB->prefix('xf_groups') . " WHERE group_name regexp '" . $startswith . "'" . ' AND type=1 ' . ' AND is_public=1 ' . " AND status='A' ";
    }
    ...
    if ($offset > $count) {
        return '';
    }//if they screw with the query string and set the offset greater than the count dont display anything;

    $content = '';

    $content .= "<p align='right'>";

    if ($count <= $range && 0 == $offset) {
        $range = $count;
        ...
        if (0 != $range) {//just to make sure a div by 0 does not occur.  Should never happen, but if it does, set it to the default of 10
