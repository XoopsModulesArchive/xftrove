...
$GLOBALS['xoopsOption']['template_main'] = 'xftrove_trove_list.html';
...
$form_cat = (int)$form_cat;
if ($form_cat != $TROVE_COMMUNITY) {
    // get info about current folder

    $res_trove_cat = $xoopsDB->query(
        'SELECT * ' . 'FROM ' . $xoopsDB->prefix('xf_trove_cat') . ' ' . "WHERE trove_cat_id='$form_cat'"
    );
...
if ($discrim) {
    ...
    for ($i = 0; $i < $lims; $i++) {
        ...
        $expl_discrim[$i] = (int)$expl_discrim[$i];
        ...
        $discrim_queryand .= 'AND trove_group_link_' . $i . '.trove_cat_id=' . $expl_discrim[$i] . ' AND trove_group_link_' . $i . '.group_id=' . '' . $xoopsDB->prefix('xf_trove_agg') . '.group_id ';
        ...
        for ($j = 0, $jMax = count($expl_discrim); $j < $jMax; $j++) {
            if ($i != $j) {
                if (!$discrim_url_b[$j]) {
    ...
    for ($i = 0, $iMax = count($expl_discrim); $i < $iMax; $i++) {
        $discrim_desc .= '<BR> &nbsp; &nbsp; &nbsp; ' . trove_getfullpath($expl_discrim[$i]) . ' <A href="' . XOOPS_URL . '/modules/xftrove/trove_list.php?form_cat=' . $form_cat . $discrim_url_b[$i] . '">[Remove This Filter]' . '</A>';
    }
...
if ($form_cat != $TROVE_COMMUNITY) {
    ...
    for ($i = 0; $i < $folders_len; $i++) {
        ...
        if ($folders_ids[$i] != $form_cat) {
            $content .= '<A href="' . XOOPS_URL . '/modules/xftrove/trove_list.php?form_cat=' . $folders_ids[$i] . $discrim_url . '">';
        } else {
            $content .= '<B>';
        }
    ...
    $res_sub = $xoopsDB->query(
        'SELECT tc.trove_cat_id AS trove_cat_id,tc.fullname AS fullname,tts.subprojects AS subprojects '
        . 'FROM '
        . $xoopsDB->prefix('xf_trove_cat')
        . ' tc LEFT JOIN '
        . $xoopsDB->prefix('xf_trove_treesums')
        . ' tts USING (trove_cat_id) '
    ...
    while (false !== ($row_sub = $xoopsDB->fetchArray($res_sub))) {
        ...
        $content .= '&nbsp; ' . $row_sub['fullname'] . '</a> <I>(' . ($row_sub['subprojects'] ?: '0') . ')</I><BR>';
    }
} else {
    $content .= '<img src="' . XOOPS_URL . '/modules/xfmod/images/ic/ofolder15.png" width="15" height="13" alt="">';
    ...
    while (false !== ($row_sub = $xoopsDB->fetchArray($result))) {
        $content .= ' &nbsp; &nbsp; ';
...
$res_rootcat = $xoopsDB->query(
    'SELECT trove_cat_id,fullname ' . 'FROM ' . $xoopsDB->prefix('xf_trove_cat') . ' ' . 'WHERE parent=0 ' . 'ORDER BY fullname'
);
//echo $xoopsDB->error();

$content2 = '';

while (false !== ($row_rootcat = $xoopsDB->fetchArray($res_rootcat))) {
    ...
    if (($row_rootcat['trove_cat_id'] == $row_trove_cat['root_parent'])
        ...
        $content2 .= '<A href="' . XOOPS_URL . '/modules/xftrove/trove_list.php?form_cat=' . $row_rootcat['trove_cat_id'] . $discrim_url . '">';
...
if ($form_cat == $TROVE_COMMUNITY) {
    $res_grp = $xoopsDB->query(
        'SELECT * ' . 'FROM ' . $xoopsDB->prefix('xf_trove_agg') . ' ' . "$discrim_queryalias " . 'WHERE ' . $xoopsDB->prefix('xf_trove_agg') . ".trove_cat_id='$comm' " . "$discrim_queryand " . 'ORDER BY ' . $xoopsDB->prefix('xf_trove_agg') . '.trove_cat_id ASC ' . ',' . $xoopsDB->prefix(
            'xf_trove_agg'
        ) . '.ranking ASC',
    ...
    $res_grp = $xoopsDB->query(
        'SELECT * ' . 'FROM ' . $xoopsDB->prefix('xf_trove_agg') . ' ' . "$discrim_queryalias " . 'WHERE ' . $xoopsDB->prefix('xf_trove_agg') . ".trove_cat_id='$form_cat' " . "$discrim_queryand " . 'ORDER BY ' . $xoopsDB->prefix('xf_trove_agg') . '.trove_cat_id ASC ' . ',' . $xoopsDB->prefix(
            'xf_trove_agg'
        ) . '.ranking ASC',
        $TROVE_HARDQUERYLIMIT,
        0
...
for ($i_proj = 1; $i_proj <= $querytotalcount; $i_proj++) {
    ...
    if ($row_grp && $viewthisrow) {
        ...
        $content4 .= "$i_proj. <a href='" . XOOPS_URL . '/modules/xfmod/project/?' . $row_grp['unix_group_name'] . "'><B>" . htmlspecialchars($row_grp['group_name'], ENT_QUOTES | ENT_HTML5) . '</B></a> ';
